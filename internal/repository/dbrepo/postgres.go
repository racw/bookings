...
// UpdateProcessedForReservation updates the processed field for a reservation
...
// DeleteByBlockID deletes a room restriction
func (m *postgresDBRepo) DeleteByBlockID(id int) error {
	ctx, cancel := context.WithTimeout(context.Background(), 3*time.Second)
	defer cancel()
	query := `
